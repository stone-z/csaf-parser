...
class CVRFParser:
    ...
    def __init__(self, parser, *args, **kwargs):
        ...
        self.progname = kwargs.get('progname')  # TODO: Remove entirely in favor of better logging configuration

        logging.info('command line args processed successfully')
        logging.info(args)

        logging.info('file to parse: ' + args.file)
        self.schema = args.schema if args.schema else CVRF_Syntax(cvrf_version).CVRF_SCHEMA_FILE
        logging.info('schema: ' + self.schema)

        self.catalog = args.catalog if args.catalog else CVRF_Syntax(cvrf_version).CVRF_CATALOG
        logging.info('catalog: ' + self.catalog)

        self.output_format = args.output_format if args.output_format else self.default_output_format
        logging.info('output format: ' + self.output_format)

        self.output_file = args.output_file if args.output_file else ''
        if self.output_file:
            logging.info('output file: ' + self.output_file)
        else:
            logging.info('output file not specified - using stdout')

        self.related_product_tags = process_related_product_tag_args(args, CVRF_Syntax(cvrf_version).related_product_tags)
        logging.info('related_product_tags: ' + ','.join(self.related_product_tags))
    ...
    def xparse(self):
        ...
        try:
            ...
            cvrf_doc = etree.parse(self.args.file, etree.XMLParser(encoding="utf-8"))  # "utf-8"
            logging.info('document successfully parsed')
        except IOError:
            logging.error("{0}: I/O error: \"{1}\" does not exist".format(progname, self.args.file))
            sys.exit("{0}: I/O error: \"{1}\" does not exist".format(progname, self.args.file))
        except etree.XMLSyntaxError as e:
            logging.error("{0}: Parsing error, document \"{1}\" is not well-formed: {2}".format(progname, self.args.file,
                                                                                                e.error_log.filter_from_level(
                                                                                                    etree.ErrorLevels.FATAL)))
            sys.exit("{0}: Parsing error, document \"{1}\" is not well-formed: {2}".format(progname, self.args.file,
                                                                                           e.error_log.filter_from_level(
                                                                                               etree.ErrorLevels.FATAL)))
        ...
        if self.args.validate is True:
            ...
            try:
                if self.args.schema:
                    ...
                    f = open(self.args.schema, 'r')

                    # If the supplied file is not a valid catalog.xml or doesn't exist lxml will fall back to using remote validation
                    catalog = self.args.catalog if self.args.catalog else CVRF_Syntax(cvrf_version).CVRF_CATALOG
                ...
                logging.error("{0}: I/O error({1}) \"{2}\": {3}".format(progname, e.errno, self.schema, e.strerror))
                sys.exit("{0}: I/O error({1}) \"{2}\": {3}".format(progname, e.errno, self.schema, e.strerror))
        ...
        cvrf_dispatch(cvrf_doc, parsables, self.args.collate_vuln, self.args.strip_ns, cvrf_version, self.output_format, self.output_file,
                      self.args,
                      self.related_product_tags)
